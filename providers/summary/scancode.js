...
const { extractDate, isLicenseFile, setIfValue, addArrayToSet, setToArray } = require('../../lib/utils')
...
class ScanCodeSummarizer {
  ...
  summarize(coordinates, harvested) {
    ...
    const declaredLicense = this._getDeclaredLicense(scancodeVersion, files, coordinates)
  ...
  _getDeclaredLicense(scancodeVersion, files, coordinates) {
    switch (scancodeVersion) {
      case '2.2.1':
        return this._getLicenseByPackageAssertion(files)
      case '2.9.2':
      case '2.9.8':
        return this._getLicenseByFileName(files, coordinates)
      case '3.0.0':
        return this._getLicenseByIsLicenseText(files)
      default:
        return null
    }
  }
